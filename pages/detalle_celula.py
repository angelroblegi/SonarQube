...
UPLOAD_DIR = "uploads"
COLUMNA_CUMPLIMIENTO_TOTAL = 'Cumplimiento Total (%)'
...
def calcular_okr_cumplimiento(df_celula, df_cobertura, config_metricas, config_na, metas, parametros, proyectos_excluir_coverage):
    """Calcular cumplimiento por métrica en función de componentes objetivo."""
    okr_data = []
    ...
    # Calcular porcentaje de cumplimiento respecto a componentes objetivo
    for metrica in ['Confiabilidad', 'Mantenibilidad', 'Complejidad', 'Cobertura']:
        if metrica in total_componentes and metrica in componentes_cumplen:
            ...
            okr_data.append({
                ...
                COLUMNA_CUMPLIMIENTO_TOTAL: cumplimiento_okr,
...
if okr_data:
    ...
    # Mostrar tabla OKR
    st.subheader("📈 Tabla de Cumplimiento por Componentes")
    
    # Formatear la tabla
    df_okr_display = df_okr.copy()
    df_okr_display['Meta Configurada (%)'] = df_okr_display['Meta Configurada (%)'].apply(lambda x: f"{x:.0f}%")
    df_okr_display[COLUMNA_CUMPLIMIENTO_TOTAL] = df_okr_display[COLUMNA_CUMPLIMIENTO_TOTAL].apply(lambda x: f"{x:.0f}%")
    ...
    **📋 Explicación del cálculo:**
    ...
    - **Cumplimiento Total**: Porcentaje de cumplimiento respecto al objetivo (Cumplen/Objetivo × 100)
    
    **Ejemplo**: Si hay 10 componentes y la meta es 90%, el objetivo es 9 componentes.
    Si cumplen 9, el cumplimiento es 100%. Si cumplen 5, el cumplimiento es 56% (5/9).
    """)
else:
...
if 'reliability_rating' in metricas_seleccionadas and not df_confiabilidad.empty:
    ...
    if not df_reliability_calc.empty:
        ...
        cumplimiento_reliability = redondear_hacia_arriba((cumplen / total) * 100) if total > 0 else 0
        cumplimiento_data.append(('Confiabilidad', cumplimiento_reliability, meta_configurada, '#ff7f0e'))
...
if 'sqale_rating' in metricas_seleccionadas and not df_mantenibilidad.empty:
    ...
    if not df_maintainability_calc.empty:
        ...
        cumplimiento_maintainability = redondear_hacia_arriba((cumplen / total) * 100) if total > 0 else 0
        cumplimiento_data.append(('Mantenibilidad', cumplimiento_maintainability, meta_configurada, '#2ca02c'))
...
if 'complexity' in metricas_seleccionadas and not df_complejidad.empty:
    ...
    if not df_duplications_calc.empty:
        ...
        cumplimiento_complexity = redondear_hacia_arriba((cumplen / total) * 100) if total > 0 else 0
        cumplimiento_data.append(('Complejidad', cumplimiento_complexity, meta_configurada, '#9467bd'))
    ...
    for i, (nombre, actual, meta, color) in enumerate(cumplimiento_data):
        with cols[i]:
            ...
            if actual >= meta:
                ...
                st.warning(f"⚠️ Meta no cumplida - Falta {faltante:.0f}%")
...
# Crear fila de resumen de cumplimiento total por métrica (cumplen/total)
...
if 'reliability_rating' in metricas_seleccionadas and not df_confiabilidad.empty:
    # En esta tabla, N/A no se cuenta en el denominador.
    ...
    if not df_temp.empty:
        total = len(df_temp)
        cumplen = int(df_temp['cumple_reliability'].sum())
        fila_resumen['Confiabilidad'] = formatear_pct((cumplen / total) * 100)
    else:
...
if 'sqale_rating' in metricas_seleccionadas and not df_mantenibilidad.empty:
    ...
    if not df_temp.empty:
        total = len(df_temp)
        cumplen = int(df_temp['cumple_maintainability'].sum())
        fila_resumen['Mantenibilidad'] = formatear_pct((cumplen / total) * 100)
    else:
        fila_resumen['Mantenibilidad'] = "N/A"

if 'coverage' in metricas_seleccionadas:
    # En esta tabla usar el mismo universo de cobertura aplicado a la métrica.
    df_temp = df_cobertura.dropna(subset=['coverage']).copy()
    ...
    if not df_temp_filtrado.empty:
        total = len(df_temp_filtrado)
        cumplen = int(df_temp_filtrado['cumple_coverage'].sum())
        fila_resumen['Cobertura de pruebas unitarias'] = formatear_pct((cumplen / total) * 100)
    else:
...
if 'complexity' in metricas_seleccionadas and not df_complejidad.empty:
    ...
    if not df_temp.empty:
        total = len(df_temp)
        cumplen = int(df_temp['cumple_duplications'].sum())
        fila_resumen['Complejidad'] = formatear_pct((cumplen / total) * 100)
    else:
...
def resaltar_cumplimiento(row):
    """Aplicar colores en fila de resumen según meta configurada por métrica."""
    estilos = [''] * len(row)
    if row['NombreProyecto'] != 'Cumplimiento (%)':
        return estilos
    ...
    for columna, meta_columna in metas_por_columna.items():
        if columna in row.index:
            valor = row[columna]
            if isinstance(valor, str) and '%' in valor and valor != 'N/A':
                try:
                    porcentaje = float(valor.replace('%', ''))
                    idx = row.index.get_loc(columna)
                    if porcentaje >= meta_columna:
...
# Aplicar colores de cumplimiento según meta configurada
df_mostrar_final_styled = df_mostrar_final_styled.apply(resaltar_cumplimiento, axis=1)
...
if not df_historico.empty and 'Mes' in df_historico.columns:
    ...
    for metrica in metricas_seleccionadas:
        ...
        for mes_fecha in sorted(df_historico['Mes'].dt.to_period('M').unique()):
            ...
            # Calcular cumplimiento histórico por componentes (cumplen/total)
            ...
            meta_configurada = metas.get(meta_key, 90)
            cumplimiento_total = redondear_hacia_arriba(valor)

            cumplimiento_por_mes.append({
                'Mes': mes_fecha.to_timestamp(),
                COLUMNA_CUMPLIMIENTO_TOTAL: cumplimiento_total
            })
        ...
        if cumplimiento_por_mes:
            ...
            fig_trend = px.line(
                df_trend,
                x='Mes',
                y=COLUMNA_CUMPLIMIENTO_TOTAL,
                title=f"Tendencia histórica de cumplimiento - {nombres_tendencias.get(metrica, metrica)}",
                markers=True
            )

            # Línea de meta configurada para la métrica
            fig_trend.add_hline(
                y=meta_configurada,
                line_dash="dash", 
                line_color="red",
                annotation_text=f"Meta: {meta_configurada:.0f}%"
            )
